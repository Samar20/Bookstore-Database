...
def addNewBooks():
    ...
    #If publisher id is not in publisher table tell user to add books only with known publishers
    else:
...
def sendMoney():
    ...
    #Error checking try block to make sure the year and month input values are correct (int)
    try:
